...
SingleTickerProviderStateMixin{
  // 다중상속으로 SingleTickerProviderStateMixin 클래스 상속
  // -> 한 가지의 애니메이션을 변경 시 적용해주는 클래스
  // -> 탭 메뉴 변경 시 컨텐츠를 슬라이딩 애니로 이동해서 보여줌
  // 참고) 다중 애니메이션 정의 클래스는 
  // TickerProviderStateMixin 을 상속받는다

  // 빙드 메서드 재정의
  @override
  Widget build(BuildContext context) {
    // 컨텐츠 진행방향이 수직이므로 Colum() 사용
    return Column(
      children: [
        // 1. 탭바 메뉴 : _buildTabBar()
         _buildTabBar(),  
        // 2. 탭 컨텐츠 영역(확장영역 사용 - Expanded()) : _buildTabBarView()
        Expanded(child: 
        _buildTabBarView(),
        ),
      ],
  ...
  void initState(){
    ...
    // length 속성: 사용할 탭 개수
    // ysync 속성: 탭변경 시 탭메뉴와 탭뷰를 연결하는 설정
