...
// 추상클래스 연습 : 사람클래스를 추상클래스
// -> 특정인물 클래스에서 사람클래스를 받아서 구현
abstract class Saram{
  // 생성자
  Saram(this.name);

  // 속성과 메서드 설계하기
  // ->> 추상클래스 메서드는 구체적으로 구현하지 않는다
  // -> 해당 클래스에서 구현
  //- > 그러나 대체적인 틀을 잡을 수 있음
  ...
  // 4. 먹다메서드 : 내용 틀잡기
  String eat() => '$name 씨는 지금 아무것도 안먹는다';
...
// 일반상속은 부모클래스의 모든 내용을 재정의 할 필요 없음
...
// class 키워드 없으면 단순 결합 클래스임(생성자 없음)
mixin Hobby {
  // 요리하다 메서드
...
// 가수 믹스인 만들기
// 믹스인에 on 키워드를 사용하면 특정클래스를 상속받은 경우에만
// 다중상속을 하도록 제한할 수 있다
...
class JunHo extends Namja with Hobby,Singer{
  ...
  // 믹스인 Singer도 부모이르모 super 키워드에 잡힌다
  // 이름과 메시지를 합쳐서 리턴
...
void testClass(){
  ...
  print(jh.age());
  print(jh.gender());
  print(jh.eat()); // 재정의 안한 메서드는 Saram의 원본 메서드가 출력됨
