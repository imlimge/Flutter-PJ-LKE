...
void main(List<String> args) {
  ...
  // 클래스의 속성값 업데이트 하기
  d1.name = '닥스훈트';
  print('1년 후 나의 새로운 강아지 종류명은 ${d1.name}이다');
  // 중요한 것은 클래스의 속성자체의 정의다
  // 값은 그것을 사용하는 사용자의 등록에 따라 달라질 수 있다


  print('내 강아지의 색은 ${d1.color}이고 나이는 ${d1.age}살이다');
  ...
  print('훈련10회 실시함 물기지수 $doit%');

  // 다른 변수에 Dog 클래스를 생성하여 사용
  // 기존 d1과 연결성은 없다
  Dog d2 = Dog();
  print('나의 강아지 종류는 ${d2.name}');
  ...
  print('나의 고양이 종을 다시 말하면 ${c1.name}이다');
  ...
  final bb; // final은 선언 후 처음할당되는 것을 상수화 함
  bb = 'bb';
  // bb = 'bbbb'; - final 재할당은 불가
  print('${aa} ${bb}');
...
-> 생성자함수는 클래스를 사용 시 가장먼저 호출되는 메서드
...
사용하고자 할 때 기본 공유를 하도록 하는 클래스 장치

- 상속 설정 방법: (extends 확장하다)
class 클래스명 extends 부모클래스명{
  구현코드..
}

-> 만약 부모클래스 멤버필드가 초기화가 필요한 경우
상속받은 클래스 생성시 반드시 부모클래스를 생성자를 먼저 호출하기 때문에
...
// 이것을 해주도록 설정하는 메서드는 
// 무조건 실행되는 클래스에 필수메서드인 생성자 메서드에 해준다
// 생성자 메서드는 클래스와 이름이 같음

// this키워드 : 클래스 내부 멤버(속성)을 지칭함
// 생성자 메서드(){} -> 소괄호에 콤마로 초기값 세팅
